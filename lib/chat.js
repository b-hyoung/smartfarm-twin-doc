// 스마트팜 디지털 트윈 문서용 GPT 질의 로직 (런타임 무관).
// - app/api/chat/route.js (next dev / Node 서버) 와
// - worker.js (Cloudflare Workers 정적 배포) 가 이 함수를 공유한다.
// API 키는 서버(env)에서만 읽는다(브라우저 노출 금지).
...
/**
 * @param {unknown} body   요청 JSON ({ question, context, history })
 * @param {{ OPENAI_API_KEY?: string, LLM_MODEL?: string }} env
 * @returns {Promise<{ status: number, json: object }>}
 */
export async function answerChat(body, env) {
  try {
    const apiKey = env.OPENAI_API_KEY;
    if (!apiKey) {
      return { status: 500, json: { ok: false, message: 'OPENAI_API_KEY 미설정' } };
    }
    ...
    if (!question) {
      return { status: 400, json: { ok: false, message: '질문이 비어 있습니다.' } };
    }
    ...
    if (!response.ok) {
      ...
      return { status: 502, json: { ok: false, message: 'OpenAI 요청 실패', detail } };
    }

    const data = await response.json();
    const answer = extractAnswerText(data) || '답변 생성에 실패했습니다. 질문을 더 구체적으로 적어주세요.';
    return { status: 200, json: { ok: true, answer } };
  } catch (e) {
    return { status: 500, json: { ok: false, message: '처리 실패', detail: String(e?.message || e) } };
  }
}
